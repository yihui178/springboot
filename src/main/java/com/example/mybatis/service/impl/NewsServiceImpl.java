...
public class NewsServiceImpl extends ServiceImpl<NewsMapper, News> implements NewsService {
    ...
    public HttpResult<String> reviewNews(Long newsId, String action) {
        ...
        if (news == null || Boolean.TRUE.equals(news.getDeleted())) {
    ...
    public HttpResult<String> updateNews(NewsDTO dto) {
        if (dto.getId() == null) {
            return HttpResult.error(400, "新闻ID不能为空");
        }

        News existingNews = this.getById(dto.getId());
        if (existingNews == null || Boolean.TRUE.equals(existingNews.getDeleted())) {
            return HttpResult.error(404, "新闻不存在");
        }
